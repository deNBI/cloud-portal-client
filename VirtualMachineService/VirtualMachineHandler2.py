...
class VirtualMachineHandler(Iface):
    ...
    def get_images(self):
        return util.convert_openstack_to_thrift_images(openstack_images=self.openstack_connector.get_images())

    def get_image(self, openstack_id):
        return util.convert_openstack_to_thrift_image(
            openstack_image=self.openstack_connector.get_image(name_or_id=openstack_id))

    def get_public_images(self):
        return util.convert_openstack_to_thrift_images(openstack_images=self.openstack_connector.get_public_images())

    def get_private_images(self):
        return util.convert_openstack_to_thrift_images(openstack_images=self.openstack_connector.get_private_images())
    ...
    def get_flavors(self):
        return util.cv_os_to_thrift_flavors(openstack_flavors=self.openstack_connector.get_flavors())

    def get_volume(self, volume_id):
        return util.cv_os_to_thrift_volume(openstack_volume=self.openstack_connector.get_volume(name_or_id=volume_id))

    def get_volumes_by_ids(self, volume_ids):
        volumes = []
        for id in volume_ids:
            volumes.append(
                util.cv_os_to_thrift_volume(openstack_volume=self.openstack_connector.get_volume(name_or_id=id)))
        return volumes
    ...
    def import_keypair(self, keyname, public_key):
        return self.openstack_connector.import_keypair(keyname=keyname, public_key=public_key)
    ...
    def reboot_server(self, openstack_id, reboot_type):
        return self.openstack_connector.reboot_server(openstack_id=openstack_id, reboot_type=reboot_type)
    ...
    def start_server(self, flavor, image, public_key, servername, metadata, research_environment, volume_ids_path_new,
                     volume_ids_path_attach, additional_keys):
        if research_environment:
            research_environment_metadata = self.forc_connector.get_metadata_by_research_environment(
                research_environment=research_environment)
        else:
            research_environment_metadata = None
        return self.openstack_connector.start_server(flavor=flavor, image=image, public_key=public_key,
                                                     servername=servername,
                                                     metadata=metadata, volume_ids_path_new=volume_ids_path_new,
                                                     volume_ids_path_attach=volume_ids_path_attach,
                                                     additional_keys=additional_keys,
                                                     research_environment_metadata=research_environment_metadata)

    def start_server_with_custom_key(self, flavor, image, servername, metadata, research_environment,
        ...
        if research_environment:
            ...
            research_environment_metadata = None
        openstack_id, private_key = self.openstack_connector.start_server_with_playbook(flavor=flavor, image=image,
                                                                                        servername=servername,
                                                                                        metadata=metadata,
                                                                                        research_environment_metadata=research_environment_metadata,
                                                                                        volume_ids_path_new=volume_ids_path_new,
                                                                                        volume_ids_path_attach=volume_ids_path_attach)
        self.forc_connector.set_vm_wait_for_playbook(openstack_id=openstack_id, private_key=private_key,
                                                     name=servername)
        return openstack_id
    ...
    def add_cluster_machine(self, cluster_id, cluster_user, cluster_group_id, image, flavor, name, key_name, batch_idx,
                            worker_idx):
        return self.openstack_connector.add_cluster_machine(cluster_id=cluster_id, cluster_user=cluster_user,
                                                            cluster_group_id=cluster_group_id,
                                                            image=image, flavor=flavor, name=name, key_name=key_name,
                                                            batch_idx=batch_idx, worker_idx=worker_idx)
