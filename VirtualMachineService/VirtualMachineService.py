...
class Iface(object):
    ...
    def add_cluster_machine(self, cluster_id, cluster_user, cluster_group_id, image, flavor, name, key_name, batch_idx, worker_idx, pub_key):
        """
        Parameters:
         - cluster_id
         - cluster_user
...
class Client(Iface):
    ...
    def add_cluster_machine(self, cluster_id, cluster_user, cluster_group_id, image, flavor, name, key_name, batch_idx, worker_idx, pub_key):
        ...
        """
        self.send_add_cluster_machine(cluster_id, cluster_user, cluster_group_id, image, flavor, name, key_name, batch_idx, worker_idx, pub_key)
        return self.recv_add_cluster_machine()

    def send_add_cluster_machine(self, cluster_id, cluster_user, cluster_group_id, image, flavor, name, key_name, batch_idx, worker_idx, pub_key):
...
class Processor(Iface, TProcessor):
    ...
    def process_add_cluster_machine(self, seqid, iprot, oprot):
        ...
        result = add_cluster_machine_result()
        try:
            result.success = self._handler.add_cluster_machine(args.cluster_id, args.cluster_user, args.cluster_group_id, args.image, args.flavor, args.name, args.key_name, args.batch_idx, args.worker_idx, args.pub_key)
            msg_type = TMessageType.REPLY
        except TTransport.TTransportException:
            raise
        except TApplicationException as ex:
            ...
            if fid == 0:
                if ftype == TType.MAP:
                    ...
                    for _i459 in range(_size455):
                        _key460 = iprot.readString().decode('utf-8', errors='replace') if sys.version_info[0] == 2 else iprot.readString()
                        _val461 = iprot.readString().decode('utf-8', errors='replace') if sys.version_info[0] == 2 else iprot.readString()
...
class add_cluster_machine_args(object):
    ...
    def __init__(self, cluster_id=None, cluster_user=None, cluster_group_id=None, image=None, flavor=None, name=None, key_name=None, batch_idx=None, worker_idx=None, pub_key=None,):
        self.cluster_id = cluster_id
