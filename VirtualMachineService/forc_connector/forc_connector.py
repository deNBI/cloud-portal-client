...
from VirtualMachineService.forc_connector.playbook.Playbook import Playbook
from VirtualMachineService.forc_connector.template.Template import Template
from VirtualMachineService.logger import setup_custom_logger
import yaml
import redis

from VirtualMachineService.util import VmTaskStates
...
class ForcConnector:
    ...
    def __init__(self, config_file):
        ...
        self.template = Template(github_playbook_repo=self.GITHUB_PLAYBOOKS_REPO, forc_url=self.FORC_URL,
    ...
    def set_vm_wait_for_playbook(self, openstack_id, private_key, name):
        self.redis_connection.hmset(
            openstack_id,
            dict(key=private_key, name=name, status=VmTaskStates.PREPARE_PLAYBOOK_BUILD),
        )
    ...
    def get_playbook_status(self, server):
        ...
        if self.redis_connection.exists(openstack_id) == 1:
            ...
            status = self.redis_connection.hget(
                openstack_id, "status"
            ).decode("utf-8")
    ...
    def get_metadata_by_research_environment(self, research_environment):
        if research_environment in self.template.get_loaded_resenv_metadata():
            ...
            return resenv_metadata
                ...
                and research_environment != BIOCONDA
        ):
            logger.error(
    ...
    def create_and_deploy_playbook(
        ...
        playbook = Playbook(
            ...
            loaded_metadata_keys=list(self.template.get_loaded_resenv_metadata().keys()),
